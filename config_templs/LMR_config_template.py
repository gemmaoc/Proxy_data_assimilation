...
class psm(ConfigGroup):
    ...
    class bayesreg_mgca(ConfigGroup):
        ...
        psm_seatemp_variable : dict
            ...
            and str value indicating whether full field (`full`) or anomaly
            (`anom`) is to be used.
        psm_salinity_variable : dict
            Required sea-surface salinity state variable for the PSM to
            calculate Ye. The dict has a str key giving the state variable name
            and str value indicating whether full field (`full`) or anomaly
            (`anom`) is to be used.
        psm_ph_variable : dict
            Optional seawater pH state variable for the PSM to calculate Ye.
            The dict has a str key giving the state variable name
            and str value indicating whether full field (`full`)
            or anomaly (`anom`) is to be used. Will be passed to
            ``baymag.predict_mgca``.
        ...
        """
        psm_seatemp_variable = {'tos_sfc_Odecmon': 'full'}
        psm_salinity_variable = {'sos_sfc_Odecmon': 'full'}
        psm_omega_variable = {}
        psm_ph_variable = {}
        seasonal_seatemp = True
        seawater_age = None
        ...
        def psm_required_variables(self):
            ...
            used.
            """
            return {**self.psm_seatemp_variable, **self.psm_salinity_variable,
                    **self.psm_ph_variable, **self.psm_omega_variable}
