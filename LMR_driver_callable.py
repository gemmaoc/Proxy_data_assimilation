...
def LMR_driver_callable(cfg=None):
    ...
    #ob_err_adjust = core.ob_err_adjust #commented out by GKO on 3/9/20 because not in my config file...
    revert_to_prior = True #core.revert_to_prior #hard coded because didn't like how I added this to the config file. not sure why it didn't update when i merged with Robert's updates. 
    ...
    if not online:
        ...
        if len(invalid_Ye) > 0:
            raise SystemExit('\nInvalid Ye values detected. Revisit prior/PSM characteristics, '
                             ...
                             '\n{}\nTotal:{}'.format(invalid_Ye,len(invalid_Ye)))
    ...
    np.savez(filen, Xb_one=out_Xb_one, Xb_one_aug=out_Xb_one_aug,
             ...
             #recon_months=core.recon_months) #commented out by GKO on 3/9/20. not sure what this is. not in my config. 
    ...
    for yr_idx, t in enumerate(range(recon_period[0], recon_period[1]+1, recon_timescale)):
        ...
        for proxy_idx, Y in enumerate(prox_manager.sites_assim_proxy_objs()):
            print('Working on proxy ',proxy_idx)
            ...
            print('--------------- Processing proxy: ' + Y.id) #Added by Gemma
            if verbose > 1:
            ...
            ob_err = Y.psm_obj.R
            print('Found R for this proxy record!')
