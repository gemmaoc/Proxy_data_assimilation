...
def get_data_closest_gridpt(data,lon_data,lat_data,lon_pt,lat_pt,getvalid=None):
    ...
    if getvalid:
        ...
        while not valid and distpt < maxdist:
            ...
            # to handle if masked array
            if np.ma.is_masked(data):
                fixed_data = data.filled(fill_value=np.nan)
            else:
                fixed_data = data
            
            if len(inds) == 2:
                test_valid = np.isfinite(fixed_data[:,inds[0],inds[1]])
                test_valid2 = np.all(fixed_data[:,inds[0],inds[1]])
            elif len(inds) == 1:                
                test_valid = np.isfinite(fixed_data[inds,:])
                test_valid2 = np.all(fixed_data[inds,:])                
            else:
            ...
            if np.all(test_valid) and test_valid2:
                valid = True
                break
...
def ensemble_stats(cfg_core, info_intervals, y_assim, y_eval=None):
    ...
    npzfile = np.load(prior_filn,allow_pickle=True)
    npzfile.files
...
def assimilated_proxies(workdir):
    ...
    assimilated_proxies = np.load(apfile,allow_pickle=True)
...
def create_precalc_ye_filename(config,psm_key,prior_kind):
    ...
    if psm_key == 'linear':
        ...
        print('Calib avg period:',calib_avgPeriod)
        calib_str = ''
        state_vars_for_ye = {psm_key.split(',')[1]: prior_kind}
        #psmkey_str = psm_key.split(',')[0]
        print('state_vars_for_ye:',state_vars_for_ye)
        print('psmkey_str',psmkey_str)
    ...
    if calib_avgPeriod:
        ...
        print('calib_avgPeriod',psm_str)
    else:
        psm_str = psmkey_str + '-' + calib_str
        print('no calib_avgPeriod',psm_str)
...
def load_precalculated_ye_vals_psm_per_proxy(config, proxy_manager, proxy_set, sample_idxs):
    ...
    for psm_key in psm_keys:
        ...
        precalc_files[psm_key] = np.load(os.path.join(load_dir, load_fname),allow_pickle=True)
...
#>>>>>>> ea212b8664b415cb895130adf2c57704487461f0
        ...
        print('pidx ',pidx,' sample_idxs ',sample_idxs)
...
def load_precalculated_ye_vals(config, proxy_manager, sample_idxs):
    ...
    precalc_file = np.load(os.path.join(load_dir, load_fname),allow_pickle=True)
