...
datadir_output = datadir_input+'/'+nexp # if want to keep things tidy
...
def main():
    ...
    if datadir_output != '.':
        figdir = datadir_output+'/VerifFigs_proxy'
    ...
    if make_plots_individual_sites:
        ...
        # set-up dicts for verification stats to sort later
        tab_assim = {}
        tab_verif = {}
        tab_assim
        for per in verif_period:
            periodtag = str(per[0])+'-'+str(per[1])
            tab_assim[periodtag] = {}
            tab_verif[periodtag] = {}
        ...
        for site in assimilated_sites:
            ...
            AssimReconCR = np.zeros([nbperiods],dtype=float)
            ...
            VerifReconCR = np.zeros([nbperiods],dtype=float)
            ...
            for p in range(nbperiods):
                ...
                AssimReconCR[p] = '{:.2f}'.format(np.mean(assim_dict[p][site]['MCensCalRatio']))
                ...
                try:
                    ...
                    VerifReconCR[p] = '{:.2f}'.format(np.mean(assim_dict[p][site]['MCensCalRatio']))
                    ...
                    VerifReconCR[p] = '{:.2f}'.format(np.nan)
            ...
            # fill-in the dicts with verification metrics for later sorting
            for p in range(nbperiods):
                periodtag = str(verif_period[p][0])+'-'+str(verif_period[p][1])
                tab_assim[periodtag][sitename] = (AssimReconR[p], AssimReconCE[p], AssimReconCR[p])
                tab_verif[periodtag][sitename] = (VerifReconR[p], VerifReconCE[p], VerifReconCR[p])

        # now sort on different metrics
        sortkeys = {0: 'sorted_by_correlation', 1: 'sorted_by_ce'}

        for per in verif_period:
            periodtag = str(per[0])+'-'+str(per[1])
            for m in list(sortkeys.keys()):
                outtag = sortkeys[m]
                # take nan's out
                tab_assim_per = {k: v for k, v in tab_assim[periodtag].items() if np.isfinite(v[m])}
                tab_verif_per = {k: v for k, v in tab_verif[periodtag].items() if np.isfinite(v[m])}
                # sort
                sorted_assim = sorted(tab_assim_per.items(), reverse=True, key=lambda x: x[1][m])
                sorted_verif = sorted(tab_verif_per.items(), reverse=True, key=lambda x: x[1][m])
                # ----------------------------------
                # output to ascii file, table format
                # ----------------------------------
                # assimilated records
                fname = figdir+'/proxies_assimilated_'+outtag+'_'+periodtag+'.txt'
                fout = open(fname, 'w')
                fout.write("%-4s %-110s %8s %8s %8s\n" %('#', 'proxy_record', 'r', 'CE', 'CalRatio'))
                for i in range(len(sorted_assim)):                
                    fout.write("%4d %-110s %8.2f %8.2f %8.2f\n" %(i+1, sorted_assim[i][0],
                                                                  sorted_assim[i][1][0],   # r 
                                                                  sorted_assim[i][1][1],   # CE
                                                                  sorted_assim[i][1][2],   # cal. ratio
                    ))
                fout.close()
                # withheld records
                if verif_data:
                    fname = figdir+'/proxies_withheld_'+outtag+'_'+periodtag+'.txt'
                    fout = open(fname, 'w')
                    fout.write("%-4s %-110s %8s %8s %8s\n" %('#', 'proxy_record', 'r', 'CE', 'CalRatio'))
                    for i in range(len(sorted_verif)):
                        fout.write("%4d %-110s %8.2f %8.2f %8.2f\n" %(i+1, sorted_verif[i][0],
                                                                      sorted_verif[i][1][0],   # r 
                                                                      sorted_verif[i][1][1],   # CE
                                                                      sorted_verif[i][1][2],   # cal. ratio
                        ))
                    fout.close()
